...
const Chat: React.FC = () => {
  ...
  const selectChat = async (chatId: string) => {
    try {
      ...
      const chatMessages: Message[] = Array.isArray(rawMessages)
        ? rawMessages.map(
            (msg: any): Message => ({
              content: msg?.content ?? "",
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await api.post("/chat", {
        messages: [
          ...messages.map((msg) => ({ content: msg.content, role: msg.role })),
          { content: userMessage.content, role: "user" },
        ],
        model: selectedModel,
      });
  ...
  return (
    <div className="flex h-[calc(100vh-120px)]">
      {/* Sidebar */}
      <div
        className={`${sidebarOpen ? "w-80" : "w-0"} transition-all duration-300 overflow-hidden bg-gray-900 border-r border-gray-700`}
      >
